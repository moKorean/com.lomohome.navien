...
> 영문 README는 [`README.md`](./README.md)를 참고하세요.
...
- **에어원 제어** — 전원, 운전 모드(자동·환기·환기제습·제습·청정·요리·수면·바이패스),
  풍량(자동·절전·미풍·강풍·터보), 목표 습도(제습 계열, 서버 범위로 검증).
- **공기질 센서** — 극초미세먼지(PM1.0)·초미세먼지(PM2.5)·미세먼지(PM10)·CO₂·TVOC·라돈·
  온습도·종합 공기질 점수·필터 사용률. TVOC·라돈은 "좋음/나쁨" 등급 라벨도 함께 표시.
  수치는 모두 숫자로 파싱되어 Homey Insights 그래프로 남습니다.
- **에어모니터** — 페어링 시 별도 센서 기기로 등록.
- **숙면매트** — 전원, 구역별 온도(0.5℃)·단계, 싱글·더블, 사계절 난방/냉방, 운전·오류 상태,
  고온 경고.
- **Homey 플로우** — 읽는 값은 조건 카드로, 운전 모드·풍량·전원·목표 습도는 동작 카드로.
- **모드별 조작 안내** — 현재 모드에서 지원하지 않는 조작(예: 자동 모드의 습도 조절)은 조용히
  실패하지 않고 안내 문구(토스트)를 띄우고 값을 되돌립니다.
- **계정 로그인** — 앱 설정 또는 기기 추가 과정에서 로그인하면 자격증명이 저장됩니다.
  다중 home 선택, 비밀번호 변경 시 복구(repair)도 지원.

## 지원 범위

| 기기 | 상태 |
| --- | --- |
| 에어원 — 전원·운전 모드·풍량·목표 습도 | 지원 (실기기 제어 확인) |
| 에어원 — 공기질(PM·CO₂·TVOC·라돈·온습도·종합등급·필터·오류) | 지원 |
| 에어원 — 모드/풍량/옵션을 서버 지원값으로 좁힘 | 지원 |
...
3. 기기 추가 → **나비엔 에어원 / 에어모니터 / 숙면매트** → 계정의 기기 목록에서 선택합니다.
   ...
   - 앱 설정에서 로그인하지 않았다면, 기기 추가 화면에서 로그인해도 자격증명이 저장됩니다.
