...
리버스 엔지니어링 구현이다. 매트(serviceCode 200)는 실기기 검증, 에어원(300, 환기·제습·청정)은
앱에서 규약을 추출하고 상태 수신은 제보로 확인, 제어는 미검증.
...
- **running(운전상태):** 1=운전(ON) / 2=정지(OFF) / 3=외출(AWAY). `roomController.running` 우선, 없으면 `odu.running`.
